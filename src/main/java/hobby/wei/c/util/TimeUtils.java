...
public class TimeUtils {
    ...
    public static String getTimeAgoCN(long timeMillis) {
    ...
    public static String getTimeAgo(long timeMillis, String yearUnit, String monthUnit, String dayUnit, String hourUnit, String minuteUnit, String justNow, String before) {
        return getTimeAgo(timeMillis, yearUnit, yearUnit, null, monthUnit, monthUnit, dayUnit, dayUnit, hourUnit, hourUnit, minuteUnit, minuteUnit, justNow, before);
    }

    public static String getTimeAgo(long timeMillis, String yearUnit, String yearsUnit, String yearMonthSpace, String monthUnit, String monthsUnit, String dayUnit, String daysUnit,
                                    String hourUnit, String hoursUnit, String minuteUnit, String minutesUnit, String justNow, String before) {
        ...
        if (timeYMD >= REACH_YEAR) {
            ...
            return year + (year == 1 ? yearUnit : yearsUnit) + (month == 0 ? "" : ((yearMonthSpace == null ? "" : yearMonthSpace) +
                    month + (month == 1 ? monthUnit : monthsUnit))) + before;
        } else if (timeYMD >= REACH_MONTH) {
            int month = (int) (timeYMD / REACH_MONTH);
            return month + (month == 1 ? monthUnit : monthsUnit) + before;
        } else if (timeYMD >= REACH_DAY) {
            int day = (int) (timeYMD / REACH_DAY);
            return day + (day == 1 ? dayUnit : daysUnit) + before;
        } else if (timeYMD >= REACH_HOUR) {
            int hour = (int) (timeYMD / REACH_HOUR);
            return hour + (hour == 1 ? hourUnit : hoursUnit) + before;
        } else if (timeYMD >= REACH_MINUTE) {
            int minute = (int) (timeYMD / REACH_MINUTE);
            if (minute >= 5) {
                return minute + (minute == 1 ? minuteUnit : minutesUnit) + before;
            } else {
                return justNow;
            }
        } else {
    ...
    public static int getDayNoon() {
        return getDayNoon(System.currentTimeMillis());
    }

    public static int getDayNoon(long timeMillis) {
        int hour = Integer.valueOf(format(timeMillis, "HHmm", false), 10);
