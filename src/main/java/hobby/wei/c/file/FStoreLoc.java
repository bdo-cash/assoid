...
public class FStoreLoc {
    ...
    public void setBaseDirName(Context context, String baseDir) {
        ...
        if (baseDirName != null) {
            if (!baseDirName.equals(baseDir)) L.i(TAG, "不可重复设置不同的目录");
            return;
        }
        baseDirName = baseDir;
        saveCurrent(context);
    ...
    private SdCard getAnother(SdCard current) {
        ...
        L.i(TAG, "getAnother----sdcardTo:%s", sdcardTo);
        return sdcardTo;
    }
    ...
    private static TAG.LogTag TAG = new TAG.LogTag(FStoreLoc.class.getName());
