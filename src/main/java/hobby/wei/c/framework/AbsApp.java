...
public abstract class AbsApp extends Application {
    ...
    private static final TAG.LogTag TAG = new TAG.LogTag(AbsApp.class.getName());
...
//    /*package*/ IUserConfig<?> mUserConfig;
     ...
     */
    protected boolean onExit(boolean firstLaunch) {
        return false;
    }

    private void ensureInfos() {
        mFirstLaunch = isFirstLaunch(0);
    }
    ...
    /*package*/ void doExit() {
        if (onExit(mFirstLaunch) && checkCallingOrSelfPermission(android.Manifest.permission.KILL_BACKGROUND_PROCESSES) == PackageManager.PERMISSION_GRANTED) {
            L.w(TAG, "@@@@@@@@@@@@@@@@@@---程序关闭---killBackgroundProcesses");
        ...
        mForceExit = false;
        mFirstLaunch = false;
        L.w(TAG, "@@@@@@@@@@@@@@@@@@---程序关闭---没有killBackgroundProcesses");
    }
     ...
     */
    //private final WeakHashMap<AbsActivity, Object> mActivities = new WeakHashMap<AbsActivity, Object>();
    private final Stack<WeakReference<? extends AbsActivity>> mActivitieStack = new Stack<WeakReference<? extends AbsActivity>>();
}
